...
function WalletButton() {
  ...
  const handleAirdrop = async (e) => {
    ...
    try {
      ...
      const bal = await connection.getBalance(publicKey);
      setBalance(bal / LAMPORTS_PER_SOL);
      setAirdropMsg('✅ 2 SOL dropped!');
    } catch (err) {
      const msg = err?.message || '';
      if (msg.includes('429') || msg.toLowerCase().includes('rate')) {
        setAirdropMsg('⚠️ Rate limited, try again in 24h');
      } else {
  ...
  if (connected && publicKey) {
    ...
    return (
      <div className="wallet-connected-group">
        {airdropMsg && <span className="airdrop-msg">{airdropMsg}</span>}
        <button className="airdrop-btn" onClick={handleAirdrop} disabled={airdropping}>
          {airdropping ? '⏳ Dropping…' : '🪂 2 SOL'}
        </button>
        <button className="btn-connect connected" onClick={() => disconnect()} title="Click to disconnect">
          {balance !== null && <span className="wallet-bal">{balance.toFixed(3)} SOL ·</span>}
    ...
    <div className="page">
      <div className="launch-wrap">
        ...
        <div className="launch-form">
          ...
          {!connected && <div className="wallet-required-notice">⚠️ Connect Phantom wallet to launch</div>}
          {launchError && <div className="launch-error-notice">❌ {launchError}</div>}
          <button className="btn-bomb" onClick={handleLaunch} disabled={launching}>
            {launching ? '⏳ Creating mint on devnet…' : connected ? '💣 Drop Bomb · Launch Token' : '🔗 Connect Wallet to Launch'}
          </button>
...
function TradingPanel({ token }) {
  ...
  const handleTrade = async () => {
    ...
    setTxStatus({state:'error', msg:'Smart contract not yet deployed on devnet. Coming soon!'});
  };
  ...
  return (
    <div className="trading-panel">
      ...
      {mode==='buy'
        ? <button className="btn-buy-now" onClick={handleTrade}>{connected ? '🟢 Buy Now' : '🔗 Connect to Buy'}</button>
        : <button className="btn-sell-now" onClick={handleTrade}>{connected ? '🔴 Sell Now' : '🔗 Connect to Sell'}</button>
      }
