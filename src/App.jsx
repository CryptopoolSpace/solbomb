...
const IDL = {"version":"0.1.0","name":"solbomb","instructions":[{"name":"initialize","accounts":[{"name":"config","isMut":true,"isSigner":false},{"name":"admin","isMut":true,"isSigner":true},{"name":"feeReceiver","isMut":false,"isSigner":false},{"name":"systemProgram","isMut":false,"isSigner":false}],"args":[{"name":"feeBps","type":"u64"}]},{"name":"create","accounts":[{"name":"bondingCurve","isMut":true,"isSigner":false},{"name":"tokenVault","isMut":true,"isSigner":false},{"name":"solVault","isMut":true,"isSigner":false},{"name":"mint","isMut":true,"isSigner":false},{"name":"config","isMut":true,"isSigner":false},{"name":"creator","isMut":true,"isSigner":true},{"name":"tokenProgram","isMut":false,"isSigner":false},{"name":"associatedTokenProgram","isMut":false,"isSigner":false},{"name":"systemProgram","isMut":false,"isSigner":false},{"name":"rent","isMut":false,"isSigner":false}],"args":[{"name":"name","type":"string"},{"name":"symbol","type":"string"},{"name":"uri","type":"string"}]},{"name":"buy","accounts":[{"name":"bondingCurve","isMut":true,"isSigner":false},{"name":"solVault","isMut":true,"isSigner":false},{"name":"tokenVault","isMut":true,"isSigner":false},{"name":"mint","isMut":true,"isSigner":false},{"name":"buyerAta","isMut":true,"isSigner":false},{"name":"config","isMut":true,"isSigner":false},{"name":"feeReceiver","isMut":true,"isSigner":false},{"name":"buyer","isMut":true,"isSigner":true},{"name":"tokenProgram","isMut":false,"isSigner":false},{"name":"associatedTokenProgram","isMut":false,"isSigner":false},{"name":"systemProgram","isMut":false,"isSigner":false}],"args":[{"name":"solAmount","type":"u64"},{"name":"minTokens","type":"u64"}]},{"name":"sell","accounts":[{"name":"bondingCurve","isMut":true,"isSigner":false},{"name":"solVault","isMut":true,"isSigner":false},{"name":"tokenVault","isMut":true,"isSigner":false},{"name":"mint","isMut":true,"isSigner":false},{"name":"sellerAta","isMut":true,"isSigner":false},{"name":"config","isMut":true,"isSigner":false},{"name":"feeReceiver","isMut":true,"isSigner":false},{"name":"seller","isMut":true,"isSigner":true},{"name":"tokenProgram","isMut":false,"isSigner":false},{"name":"associatedTokenProgram","isMut":false,"isSigner":false},{"name":"systemProgram","isMut":false,"isSigner":false}],"args":[{"name":"tokenAmount","type":"u64"},{"name":"minSol","type":"u64"}]},{"name":"updateConfig","accounts":[{"name":"config","isMut":true,"isSigner":false},{"name":"admin","isMut":true,"isSigner":true}],"args":[{"name":"newFeeBps","type":"u64"}]}],"accounts":[{"name":"Config","type":{"kind":"struct","fields":[{"name":"admin","type":"publicKey"},{"name":"feeReceiver","type":"publicKey"},{"name":"feeBps","type":"u64"},{"name":"graduationSol","type":"u64"},{"name":"totalLaunched","type":"u64"},{"name":"totalVolume","type":"u64"},{"name":"bump","type":"u8"}]}},{"name":"BondingCurve","type":{"kind":"struct","fields":[{"name":"mint","type":"publicKey"},{"name":"creator","type":"publicKey"},{"name":"virtualSol","type":"u64"},{"name":"virtualTokens","type":"u64"},{"name":"realSol","type":"u64"},{"name":"realTokens","type":"u64"},{"name":"totalSupply","type":"u64"},{"name":"graduated","type":"bool"},{"name":"createdAt","type":"i64"},{"name":"name","type":"string"},{"name":"symbol","type":"string"},{"name":"uri","type":"string"},{"name":"curveBump","type":"u8"},{"name":"tokenVaultBump","type":"u8"},{"name":"solVaultBump","type":"u8"}]}}],"errors":[{"code":6000,"name":"SlippageExceeded","msg":"Slippage exceeded"},{"code":6001,"name":"AlreadyGraduated","msg":"Already graduated"},{"code":6002,"name":"InsufficientSol","msg":"Insufficient SOL"},{"code":6003,"name":"InsufficientTokens","msg":"Insufficient tokens"},{"code":6004,"name":"InvalidFee","msg":"Invalid fee"},{"code":6005,"name":"Unauthorized","msg":"Unauthorized"},{"code":6006,"name":"Overflow","msg":"Overflow"},{"code":6007,"name":"ZeroAmount","msg":"Zero amount"}]};
...
// ── Anchor Program Helper — lazy init only when needed ────────────────────────
...
function LaunchPage({ onSuccess }) {
  ...
  const handleLaunch = async () => {
    ...
    setLaunching(true);
    setTxStatus({ state: 'loading', msg: '⏳ Initializing program…' });
    ...
    try {
      ...
      setTxStatus({ state: 'error', msg: `❌ Program error: ${err.message}` });
      setLaunching(false);
      return;
    }
    ...
    try {
      ...
      const tokenVault = await getAssociatedTokenAddress(mint, solVault, true);

      setTxStatus({ state: 'loading', msg: '⏳ Launching token on-chain…' });

      const uri = `https://gateway.pinata.cloud/ipfs/bafkreieuca7nuxhprr3psacgphtsvkbycjobez3vrikoyh3ounjuwhxili`;

      const tx = await program.methods
        .create(name.trim(), sym.trim().toUpperCase(), uri)
        .accounts({
          bondingCurve,
          tokenVault,
          solVault,
      ...
      setTxStatus({ state: 'success', msg: `✅ Token launched! TX: ${tx.slice(0,16)}…` });
      setLaunching(false);
    ...
    <div className="page-content">
      <div className="launch-wrap">
        <div className="launch-header">
          ...
          <div className="launch-sub">Launch your memecoin on Solana devnet</div>
        </div>
        <div className="launch-form">
          ...
          <button className="btn-bomb" onClick={handleLaunch} disabled={launching}>
            {launching ? '⏳ Launching on-chain…' : connected ? '💣 Drop Bomb' : '◎ Connect to Launch'}
          </button>
        </div>
      </div>
    </div>
  ...
  useEffect(() => {
    ...
    const fetchBalances = async () => {
      ...
      try {
        ...
        console.log('SOL fetch error:', e);
        setBalance('0');
      }

      // Fetch SOLBOMBDEV balance
      try {
        ...
        if (tokenAccounts.value.length > 0) {
          ...
          setSolbombBal(uiAmount.toLocaleString());
        } else {
          setSolbombBal('0');
        }
      } catch (e) {
        console.log('SOLBOMB fetch error:', e);
        setSolbombBal('—');
      }
    };
      ...
      <div className="profile-wrap">
        ...
        <div className="holdings-grid">
          <div className="holding-card">
            ...
            <div className="holding-val cyan">{balance !== null ? `${balance} SOL` : '…'}</div>
          </div>
          <div className="holding-card">
            <div className="holding-label">$SOLBOMB</div>
            <div className="holding-val orange">{solbombBal !== null ? solbombBal : '…'}</div>
          </div>
        </div>
        <div className="holdings-note">
          ◉ Devnet · Mint: {SOLBOMB_MINT.slice(0,8)}…{SOLBOMB_MINT.slice(-4)}
        </div>
    ...
    <div className="page-content">
      <div className="disclaimer-wrap">
        ...
        <div className="disclaimer-content">
          ...
          <p>The SolBomb team makes no guarantees regarding platform uptime, token value, or future development. All roadmap items are subject to change.</p>
...
function TradingPanel({ token }) {
  ...
  const handleTrade = async () => {
    ...
    if (!token.mintAddress || !token.onChain) {
      setTxStatus({ state:'error', msg:'This is a demo token. Launch a real token to trade on-chain.' });
      return;
    }
    ...
    try {
      ...
      const [bondingCurve] = PublicKey.findProgramAddressSync(
        [Buffer.from(CURVE_SEED), mint.toBuffer()], PROGRAM_ID
      );
      const [solVault] = PublicKey.findProgramAddressSync(
        [Buffer.from(SOL_VAULT_SEED), mint.toBuffer()], PROGRAM_ID
      );
      const [config] = PublicKey.findProgramAddressSync(
        [Buffer.from(CONFIG_PDA_SEED)], PROGRAM_ID
      );
      const tokenVault = await getAssociatedTokenAddress(mint, solVault, true);
      const userAta = await getAssociatedTokenAddress(mint, publicKey);

      if (mode === 'buy') {
        const solLamports = new anchor.BN(Math.floor(solAmt * LAMPORTS_PER_SOL));
        const minTokens = new anchor.BN(0); // 0% slippage for devnet

        const tx = await program.methods
          .buy(solLamports, minTokens)
          .accounts({
            bondingCurve,
            solVault,
            tokenVault,
        ...
        setTxStatus({ state:'success', msg:`✅ Bought ${tokensEst} ${token.sym}! TX: ${tx.slice(0,12)}…` });

      } else {
        const tokenAmount = new anchor.BN(Math.floor(solAmt * 1_000_000));
        const minSol = new anchor.BN(0);
        ...
        setTxStatus({ state:'success', msg:`✅ Sold! TX: ${tx.slice(0,12)}…` });
      }
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="trading-panel">
      ...
      {mode==='buy'
        ? <button className="btn-buy" onClick={handleTrade} disabled={trading}>{trading?'⏳ Buying…':connected?'🟢 Buy Now':'◎ Connect to Buy'}</button>
        ...
        <div className="detail-header">
          ...
          <div>
            ...
            <div className="detail-links">
              {token.mintAddress && (
                <a className="detail-link" href={`https://solscan.io/token/${token.mintAddress}?cluster=devnet`} target="_blank" rel="noreferrer">🔍 Solscan</a>
              )}
          ...
          <div className="detail-left">
            ...
            {token.mintAddress && (
              <div className="mint-info-box">
                <div className="mint-info-label">MINT ADDRESS</div>
                <div className="mint-info-val">{token.mintAddress}</div>
              </div>
            )}
          </div>
...
function KotHPage({ tokens, onOpenToken }) {
  ...
  useEffect(() => {
    ...
    const id = setInterval(() => {
      ...
      const newTrade = { id:Date.now(), type:isBuy?'buy':'sell', wallet:wallets[Math.floor(Math.random()*wallets.length)], token:token.name, sol, time:Date.now() };
      setTrades(prev => [newTrade,...prev].slice(0,10));
      if (isBuy && parseFloat(sol) >= 3) setWhaleAlert({ wallet:newTrade.wallet, token:token.name, sol, time:Date.now() });
              ...
              return (
                <div key={token.id} className={`koth-rank-card${i===0?' koth-rank-king':''}`} onClick={() => onOpenToken(token)}>
                  ...
                  <div className="koth-rank-bar-wrap">
                    <div className="koth-rank-track"><div className="koth-rank-fill" style={{width:`${token.curve}%`,background:token.color}}/></div>
                  </div>
                  <div className="koth-rank-footer">
                    <div className="koth-momentum">
                      <span className="koth-momentum-label">Momentum</span>
                      <div className="koth-momentum-bar"><div className="koth-momentum-fill" style={{width:`${momentum}%`}}/></div>
  ...
  return (
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal-box" onClick={e => e.stopPropagation()}>
        ...
        {mintAddress && (
          <div style={{fontFamily:'var(--mono)',fontSize:10,color:'var(--accent)',background:'rgba(0,194,255,0.08)',border:'1px solid rgba(0,194,255,0.2)',borderRadius:8,padding:'8px 12px',margin:'12px 0',wordBreak:'break-all'}}>
            {mintAddress}
          </div>
        )}
...
export default function App() {
  ...
  return (
    <>
      ...
      {modal && (
        <SuccessModal
          name={modal.name} sym={modal.sym} mintAddress={modal.mintAddress}
