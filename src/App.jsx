...
function LaunchPage({ onSuccess }) {
  ...
  const handleLaunch = async () => {
    ...
    if (!program) { alert('Program not loaded'); return; }

    setLaunching(true);
    setTxStatus({ state: 'loading', msg: '⏳ Creating mint account on devnet…' });
...
function TradingPanel({ token }) {
  ...
  const handleTrade = async () => {
    ...
    if (!program) { setTxStatus({ state:'error', msg:'Program not loaded' }); return; }
